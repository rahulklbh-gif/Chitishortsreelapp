...
export default function CreatePage() {
  ...
  // 3. Recording Logic (No Canvas Processing to prevent hanging)
  const startRecording = () => {
    ...
    recorder.onstop = () => {
      ...
      setSelectedFile(new File([blob], "chiti_video.webm", { type: 'video/webm' }));
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // Direct Upload of the recorded blob (Super Fast)
  ...
  // Renderer for Preview (Grid & CSS Filters)
  const renderStudioDisplay = (url?: string) => {
    ...
    return (
      <div className={gridClass}>
        ...
        {filter.vfxType === 'lightning' && <div className="absolute inset-0 bg-blue-400/20 animate-pulse pointer-events-none z-10" />}
      </div>
    );
  };
                ...
                {isCameraMode ? (
                  <>
                    <div className="flex bg-black/50 p-1 rounded-full border border-white/10">
                      {[15, 30].map(sec => (
                        <button key={sec} onClick={() => setRecordLimit(sec)} className={`px-10 py-2.5 rounded-full text-[10px] font-black ${recordLimit === sec ? 'bg-white text-black' : 'text-gray-400'}`}>{sec}S</button>
                      ))}
                    </div>
                    ...
                    {isRecording && <span className="text-red-500 font-black animate-pulse">{timeLeft}S LEFT</span>}
                  </>
                ) : (
                  <button onClick={() => setIsFinalStep(true)} className="px-14 py-4 bg-blue-600 rounded-full font-black text-xs uppercase tracking-widest shadow-xl">Next Step</button>
                )}
              </div>
            </>
          ) : (
            <div className="flex-1 p-6 flex flex-col bg-black h-full overflow-y-auto">
              <div className="flex items-center gap-4 mb-10 pt-4">
                <button onClick={() => setIsFinalStep(false)} className="p-3 bg-white/5 rounded-full"><ArrowLeft size={24}/></button>
                <h2 className="text-xl font-black italic uppercase">Post Preview</h2>
              </div>
              
              <div className="flex gap-6 mb-12">
                <div className="w-32 h-48 bg-zinc-900 rounded-[30px] overflow-hidden relative border border-white/10">
                   {renderStudioDisplay(previewUrl)}
                </div>
                <textarea 
                  placeholder="Tell your story..." 
                  className="flex-1 bg-transparent border-b border-white/10 py-4 outline-none font-bold text-sm resize-none" 
                  value={caption} 
              ...
              {isUploading && (
                <div className="mb-10 space-y-2">
                  <div className="flex justify-between text-[10px] font-black text-blue-500 uppercase italic"><span>Uploading...</span><span>{uploadProgress}%</span></div>
              ...
              <button onClick={handlePublish} disabled={isUploading} className="mt-auto w-full bg-blue-600 py-6 rounded-[30px] font-black text-lg flex items-center justify-center gap-4 shadow-xl active:scale-95 transition-all">
                {isUploading ? <Loader2 className="animate-spin" size={28}/> : <Send size={28}/>}
                <span className="uppercase tracking-tighter">Publish to Feed</span>
              </button>
            </div>
          )}
        </div>
      ...
      {showFilters && (
        <div className="absolute bottom-0 inset-x-0 bg-zinc-950 p-8 rounded-t-[50px] z-[1500] border-t border-white/5">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-black text-xs uppercase italic text-zinc-500">Camera Effects</h3>
        ...
        <div className="absolute inset-0 bg-zinc-950 z-[1600] flex flex-col">
          <div className="p-6 pt-16 flex-1 flex flex-col">
            <div className="flex justify-between items-center mb-10">
              <h2 className="text-4xl font-black italic tracking-tighter text-blue-600 uppercase">Music</h2>
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pb-20 no-scrollbar">
              {musicList.map(music => (
                <div key={music.id} className={`p-5 rounded-[35px] flex items-center justify-between border ${selectedMusic?.id === music.id ? 'bg-blue-600/10 border-blue-500' : 'bg-zinc-900 border-white/5'}`}>
                  ...
                  <button onClick={() => {setSelectedMusic(music); setShowMusic(false);}} className={`p-4 rounded-2xl ${selectedMusic?.id === music.id ? 'bg-blue-600' : 'bg-white/5'}`}><Check size={20}/></button>
                </div>
              ))}
            </div>
          </div>
