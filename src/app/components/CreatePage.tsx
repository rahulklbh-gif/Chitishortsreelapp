...
// FFmpeg utilities ko import karein (Ensure these are installed)
...
export default function CreatePage() {
  ...
  /**
   * 🚀 ENHANCED PUBLISH WITH BALANCED COMPRESSION (50% size reduction)
   */
  const handlePublish = async () => {
    ...
    try {
      // Step 1: 50-60% Compression with Ultrafast (No Hang)
      const compressedBlob = await compressVideoTo480p(selectedFile, (p) => {
        setUploadProgress(Math.floor(p.progress * 0.4)); // Map to 40%
      ...
      <div className={f.isGrid ? `grid h-full w-full grid-cols-${f.cols} grid-rows-${f.rows}` : 'h-full w-full'}>
        {[...Array(grids)].map((_, i) => (
          <video 
            ...
            style={{ 
              filter: f.style, 
              // FIXED MIRRORING: Camera mirror rahega, Preview natural rahega
