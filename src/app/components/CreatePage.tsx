...
export default function CreatePage() {
    ...
    try {
      ...
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { 
          ...
          width: { ideal: 720 }, // 720p is perfect for mobile performance
          frameRate: { ideal: 24 } 
        },
        audio: true
      });
  ...
  const startRecording = async () => {
    ...
    // Memory efficient recording settings
    const recorder = new MediaRecorder(streamRef.current, { 
      mimeType: 'video/webm;codecs=vp8',
      videoBitsPerSecond: 1500000 // 1.5 Mbps - Great quality, fast upload
    });
  ...
  /**
   * 🚀 SUPER FAST PUBLISH (ZERO HANG LOGIC)
   */
  const handlePublish = async () => {
    ...
    try {
      // Logic: No canvas processing here to prevent mobile crash.
      // Filters are applied live by the player using the 'filter_name' column.
      ...
      // Progress bar simulation for slow internet feel
      ...
      await Promise.all([
        supabase.from('posts').insert([{ 
            ...
            caption, 
            user_id: user.id, 
            user_name: user.user_metadata?.username || user.email?.split('@')[0] || 'Creator',
            filter_name: selectedFilter // Player identifies this and applies the style
        }]),
    ...
    return (
      <div className={gridClass}>
        {[...Array(gridCount)].map((_, i) => (
          <div key={i} className="relative w-full h-full overflow-hidden border-[0.5px] border-white/10">
            <video 
    ...
    <div className="fixed inset-0 bg-black text-white flex flex-col z-[999] overflow-hidden">
      ...
      {!user ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-16">
          ...
          <label className="flex items-center gap-4 px-10 py-5 bg-zinc-900 rounded-full border border-white/5 cursor-pointer active:scale-95 transition-all">
            ...
            <div className="flex-1 p-6 flex flex-col bg-black">
              ...
              {isUploading && (
                <div className="mb-10 space-y-2">
                   <div className="flex justify-between items-end text-[9px] font-black text-blue-500 uppercase italic"><span>Uploading to R2...</span><span>{uploadProgress}%</span></div>
              ...
              <button onClick={handlePublish} disabled={isUploading} className="mt-auto w-full bg-blue-600 py-6 rounded-[30px] font-black text-lg flex items-center justify-center gap-4 shadow-xl active:scale-95 transition-all disabled:opacity-50">
                ...
                <span className="uppercase tracking-tighter">{isUploading ? 'Fast Posting...' : 'Publish Now'}</span>
              </button>
            </div>
          )}
        </div>
