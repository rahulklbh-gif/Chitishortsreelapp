...
// Royalty Free Music (Fallback agar database khali ho)
const PERMANENT_MUSIC = [
  { id: 'p1', title: 'Chiti Viral Trap', audio_url: 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3' },
  { id: 'p2', title: 'Lofi Morning', audio_url: 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3' },
  { id: 'p3', title: 'Happy Upbeat', audio_url: 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-8.mp3' }
];
...
export function CreatePage() {
  ...
  const filterStyles: any = {
    ...
    mono: "grayscale(1)",
    cine: "contrast(1.4) saturate(1.4)"
  };

  // 1. Music Load Logic (Merging Database + Permanent)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (isCameraMode) { if (nav) nav.style.visibility = 'hidden'; }
    else { if (nav) nav.style.visibility = 'visible'; }
  }, [isCameraMode]);
  ...
  const startCamera = async () => {
    try {
      ...
      const stream = await navigator.mediaDevices.getUserMedia({ 
        video: { facingMode: facingMode, width: { ideal: 1280 }, height: { ideal: 720 }, aspectRatio: 0.5625 }, 
        audio: true 
      });
  ...
  // 3. AUTO-SAVE Logic (FORCE SAVE)
  const processRecordedVideo = () => {
    if (chunksRef.current.length === 0) return;
    const blob = new Blob(chunksRef.current, { type: 'video/webm' });
    const url = URL.createObjectURL(blob);
    setPreviewUrl(url);
    setSelectedFile(new File([blob], "chiti-video.webm", { type: 'video/webm' }));
  ...
  const startRecording = () => {
    ...
    recorder.onstop = processRecordedVideo; // Standard Save
  ...
  const stopRecording = () => {
    if (mediaRecorderRef.current && isRecording) {
      ...
      // Extra safety: agar onstop trigger nahi hua toh manually call karo 500ms baad
      setTimeout(() => { if(isRecording) processRecordedVideo(); }, 500);
    }
  };
    ...
    <div className="fixed inset-0 bg-black text-white flex flex-col overflow-hidden z-[999]">
      ...
      {!isCameraMode && !selectedFile ? (
          ...
          {showMusic && (
            <div className="absolute inset-0 bg-black/95 z-[1050] p-6 pt-20 animate-in slide-in-from-bottom">
              ...
              <div className="space-y-4 overflow-y-auto max-h-[60vh] no-scrollbar">
                {musicList.map(m => (
                  <div key={m.id} onClick={() => { setSelectedMusic(m); setShowMusic(false); }} className={`p-5 rounded-[25px] flex justify-between items-center ${selectedMusic?.id === m.id ? 'bg-blue-600' : 'bg-gray-900 border border-white/5'}`}>
                    <div className="flex flex-col"><span className="font-bold text-sm">{m.title}</span><span className="text-[10px] opacity-50">Royalty Free</span></div>
              ...
              <div className="flex gap-6 overflow-x-auto no-scrollbar pb-4">
                {Object.keys(filterStyles).map(f => (
                  <button key={f} onClick={() => setSelectedFilter(f)} className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-[10px] uppercase font-bold">{f}</span>
                  </button>
                ))}
              </div>
            </div>
        ...
        <div className="flex-1 flex flex-col p-6 pt-20 overflow-y-auto no-scrollbar pb-32">
          <div className="aspect-[9/16] w-full bg-gray-900 rounded-[35px] overflow-hidden mb-6 shadow-2xl relative border border-white/5">
          ...
          <textarea 
            ...
            className="w-full bg-gray-900 rounded-[25px] p-5 outline-none border border-gray-800 text-base mb-6 focus:border-blue-500"
            rows={3}
            value={caption}
            onChange={(e) => setCaption(e.target.value)}
          />
          <button onClick={() => toast.info("Publish system ready!")} className="w-full bg-blue-600 py-5 rounded-[25px] font-black text-xl shadow-xl shadow-blue-600/20 active:scale-95 transition-all">POST CHITI</button>
        </div>
      )}
