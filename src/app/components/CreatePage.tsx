...
// --- Permanent Royalty Free Music List ---
...
export function CreatePage() {
  ...
  // 1. Fetch Music & Merge with Permanent
  useEffect(() => {
    const loadMusic = async () => {
      ...
      if (data && data.length > 0) {
        ...
        setMusicList(PERMANENT_MUSIC);
      }
    };
    loadMusic();
  }, []);

  // 2. Hide Navigation Bar when Camera is Open
  useEffect(() => {
  ...
  const startCamera = async () => {
    try {
      ...
      toast.error("Camera access failed. Permissions check karein.");
    }
  };
  ...
  // 4. Force Save Logic (Automatically called when recording ends)
  ...
  const stopRecording = () => {
    if (mediaRecorderRef.current && isRecording) {
      ...
      // Safety timeout to ensure blob is processed
      setTimeout(() => { if(isRecording) processFinalVideo(); }, 600);
    }
  };
    ...
    <div className="fixed inset-0 bg-black text-white flex flex-col overflow-hidden select-none z-[999]">
      ...
      {!isCameraMode && !selectedFile ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 gap-8">
          ...
          <label className="w-full max-w-[260px] p-5 bg-gray-900 rounded-[25px] flex items-center justify-center gap-3 border border-gray-800 cursor-pointer active:bg-gray-800">
        ...
        <div className="relative flex-1 bg-black">
          ...
          <div className="absolute bottom-10 left-0 right-0 flex flex-col items-center gap-6 z-[1010]">
            {!isRecording && (
              <div className="flex bg-black/40 backdrop-blur-xl p-1 rounded-full border border-white/10">
                {[15, 30].map(s => (
                  <button key={s} onClick={() => {setRecordLimit(s); setTimeLeft(s);}} className={`px-5 py-1.5 rounded-full text-xs font-black transition-all ${recordLimit === s ? 'bg-white text-black shadow-lg' : 'text-gray-400'}`}>{s}s</button>
                ))}
              </div>
            )}
            
            <div className="flex flex-col items-center gap-2">
              <button onClick={isRecording ? stopRecording : startRecording} className={`w-20 h-20 rounded-full border-4 flex items-center justify-center transition-all ${isRecording ? 'border-red-500 scale-110' : 'border-white'}`}>
          ...
          {showMusic && (
            <div className="absolute inset-0 bg-black/95 z-[1050] p-6 pt-20 animate-in slide-in-from-bottom duration-300">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-black italic">SELECT MUSIC</h2>
              ...
              <div className="space-y-4 overflow-y-auto max-h-[65vh] no-scrollbar">
                {musicList.map(m => (
                  <div key={m.id} onClick={() => { setSelectedMusic(m); setShowMusic(false); }} className={`p-5 rounded-[25px] flex justify-between items-center transition-all ${selectedMusic?.id === m.id ? 'bg-blue-600' : 'bg-gray-900 border border-white/5 active:bg-gray-800'}`}>
                    <div className="flex flex-col">
                      <span className="font-bold text-sm">{m.title}</span>
                      <span className="text-[10px] opacity-50 uppercase tracking-widest">Royalty Free</span>
                    </div>
                    {selectedMusic?.id === m.id ? <Check size={18}/> : <Play size={18} fill="white"/>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Filters Panel */}
          {showFilters && (
            <div className="absolute bottom-0 left-0 right-0 bg-black/90 p-8 rounded-t-[40px] z-[1050] border-t border-white/10 animate-in slide-in-from-bottom">
              ...
              <div className="flex gap-6 overflow-x-auto no-scrollbar pb-4">
                {Object.keys(filterStyles).map(f => (
                  <button key={f} onClick={() => setSelectedFilter(f)} className="flex flex-col items-center gap-2">
                    <div className={`w-14 h-14 rounded-full border-2 ${selectedFilter === f ? 'border-blue-500 scale-110' : 'border-gray-700'} bg-gray-800 shadow-xl`} />
        ...
        <div className="flex-1 flex flex-col p-6 pt-20 overflow-y-auto no-scrollbar pb-32">
          ...
          <textarea 
            placeholder="Kamaal ki caption likhein... #chiti" 
            className="w-full bg-gray-900 rounded-[25px] p-5 outline-none border border-gray-800 text-base mb-6 focus:border-blue-500 transition-all"
            rows={3}
            value={caption}
            onChange={(e) => setCaption(e.target.value)}
          />
          <button onClick={() => toast.success("Posting logic ready!")} className="w-full bg-blue-600 py-5 rounded-[25px] font-black text-xl shadow-xl shadow-blue-600/30 active:scale-95 transition-all">
            POST CHITI
          </button>
        </div>
      )}
