        ...
        {[...Array(gridCount)].map((_, i) => (
          <div key={i} className="relative w-full h-full bg-zinc-900 overflow-hidden border-[0.5px] border-white/5">
            {isLive ? (
              ...
              <video ref={i === 0 ? previewVideoRef : null} src={previewUrl} autoPlay loop playsInline muted className="w-full h-full" style={videoStyle} />
            )}
          </div>
        ))}
      </div>
                ...
                {!previewUrl ? (
                  <>
                    <div className="flex bg-black/50 p-1.5 rounded-full border border-white/10 backdrop-blur-xl">
                      {[15, 30].map(d => (
                        <button key={d} onClick={() => { if(timer === 0) setDurationLimit(d)}} className={`px-7 py-2 rounded-full text-[10px] font-black transition-all ${durationLimit === d ? 'bg-white text-black' : 'text-zinc-500'} ${timer > 0 ? 'opacity-50' : ''}`}>{d}s</button>
                      ))}
                    </div>
                  ...
                  <div className="flex gap-4 w-full max-sm">
                    <button onClick={() => {setPreviewUrl(''); setTimer(0); chunksRef.current = []; setIsCameraMode(true); initCamera();}} className="flex-1 py-4 bg-zinc-900 rounded-2xl font-black uppercase text-[10px] border border-white/10">Discard</button>
