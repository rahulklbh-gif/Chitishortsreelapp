...
export function ProfilePage() {
  ...
  const handleUpdateProfile = async () => {
    const { error } = await supabase.from('profiles').update({ full_name: newName, username: newUsername.toLowerCase() }).eq('id', currentUser?.id);
    if (!error) {
    ...
    <div className="min-h-screen bg-black text-white pb-24">
      ...
      <div className="p-6 flex items-center gap-6">
        ...
        <div className="flex-1 flex justify-between px-2">
          <div className="text-center"><p className="text-lg font-black">{userPosts.length}</p><p className="text-[9px] text-gray-500 uppercase font-bold">Videos</p></div>
          <div className="text-center"><p className="text-lg font-black">{profile?.followers_count || 0}</p><p className="text-[9px] text-gray-500 uppercase font-bold">Followers</p></div>
          <div className="text-center"><p className="text-lg font-black">{profile?.following_count || 0}</p><p className="text-[9px] text-gray-500 uppercase font-bold">Following</p></div>
          <div className="text-center"><p className="text-lg font-black text-pink-500">{profile?.total_likes || 0}</p><p className="text-[9px] text-gray-500 uppercase font-bold">Likes</p></div>
        </div>
      </div>
      ...
      <div className="px-6 mb-6">
        {isEditing ? (
          <div className="space-y-3">
            <input value={newUsername} onChange={e => setNewUsername(e.target.value)} className="w-full bg-gray-900 p-2 rounded-lg border border-white/10 text-sm" placeholder="Username" />
            <div className="flex gap-2">
              <input value={newName} onChange={e => setNewName(e.target.value)} className="flex-1 bg-gray-900 p-2 rounded-lg border border-white/10 text-sm" placeholder="Full Name" />
              <button onClick={handleUpdateProfile} className="bg-blue-600 px-4 rounded-lg"><Check size={18} /></button>
            </div>
          </div>
        ) : (
          <><h2 className="text-xl font-black italic uppercase tracking-tighter">{profile?.full_name || 'User'}</h2><p className="text-[10px] text-blue-500 font-bold uppercase tracking-widest mt-1">Creator ⚡</p></>
        )}
      </div>
      ...
      {userPosts.length === 0 && (
        <div className="text-center py-20 text-gray-600 text-xs font-bold uppercase tracking-widest">No Videos Uploaded</div>
      )}
    </div>
  );
}
